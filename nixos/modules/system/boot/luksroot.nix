          ...
          echo "Waiting for your FIDO2 device..."
            fido2luks open ${
              concatStringsSep " " (flatten [
                ...
                (optional (!fido2.useToken) "--disable-token")
                (optional (fido2.credential != null) "--creds ${fido2.credential}")
                [
                  "--await-dev"
                  (toString fido2.gracePeriod)
                ]
                (optional fido2.passwordLess "--salt string:")
                (optional allowDiscards "--allow-discards")
                (optional (!fido2.passwordLess)
                  ''--password-helper 'read -rsp "FIDO2 salt for ${device}: " passphrase 1>&2 && echo "$passphrase"' ''
                )
                (optional fido2.pin "--pin --pin-prefixed")
              ])
            }
        if [ $? -ne 0 ]; then
          echo "No FIDO2 key found, falling back to normal open procedure"
  ...
  config = mkIf (luks.devices != { } || luks.forceLuksSupportInInitrd) {
    ...
    assertions = [
...

      {
        ...
        { assertions = luks.fido2Support && config.boot.initrd.systemd.enable; message = "fido2luks does not work with an systemd based initrd"; }
